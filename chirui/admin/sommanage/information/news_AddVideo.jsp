...
<%
	...
	try {
		...
		if (action.equals("add")) {
			...
			fujianpath = "uploadimg/video/" + datetime + name; //uploadimg/fujian/20090825155532.xxx
...
%>
var i = window.opener.Dvform.video.length;

window.opener.Dvform.video.length = i+1;
window.opener.Dvform.video.options[i].text="<%=fujianBH%>_<%=fujianSM%>";
window.opener.Dvform.video.options[i].value="<%=fujianBH%>";
window.opener.Dvform.video_Name.value += ","+"<%=fujianBH%>";
<%
...
var text = new Array();
var index = 0;
for(i=0;i<window.opener.Dvform.video.length;i++){
	if(window.opener.Dvform.video.options[i].value != "<%=delfujianBH%>"){
		value[index] = window.opener.Dvform.video.options[i].value;
        text[index] = window.opener.Dvform.video.options[i].text;
			index++
	}
}
window.opener.Dvform.video.length = value.length;
for(i=0;i<value.length;i++){
                window.opener.Dvform.video.options[i].text=text[i];
                window.opener.Dvform.video.options[i].value=value[i];
}
<%
}
%>
document.fileform.fujianBH.length = window.opener.Dvform.video.length;
for(j=0;j<window.opener.Dvform.video.length;j++){
document.fileform.fujianBH.options[j].text=window.opener.Dvform.video.options[j].text
document.fileform.fujianBH.options[j].value=window.opener.Dvform.video.options[j].value
}
...
function del(){
//fujianBH
var fnames = window.opener.Dvform.video_Name.value.split(",");
...
window.opener.Dvform.video_Name.value = newFname;
