  ...
  //初始化处理bean
    ...
    web.filename2 = config.getServletContext().getRealPath("/") + "WEB-INF\\Button.xml";
       System.out.println(web.filename);
        System.out.println(web.filename2);
    ...
    System.out.println("图片路径==sStyleNameAdd"+sStyleNameAdd);
    //图片日期同步,并且将图片存放在UPLOAD下面
    //动态的创建UPLOAD目录
    
    java.io.File picStoreDir = new java.io.File(sStyleNameAdd);
   // System.out.println("fadddd==sStyleNameAdd"+sStyleNameAdd);
    //do java.io.File.mkdirs()
    try{
      picStoreDir.mkdirs();
      }catch(Exception ex){
        System.out.println(" bbbbb=="+ex.toString());
      }
     System.out.println(" ccccc==sStyleNameAdd"+sStyleNameAdd);
    //output sStyleNameAdd path
    //out.println(sStyleNameAdd);
    //此处设置上传文件保存路径,注意路径要由ROOT开始=========================
    sStyleUploadDir = "/crjd/"+sUploadFilePath + calendar.get(Calendar.YEAR) +
    "/" + (calendar.get(Calendar.MONTH) + 1) + "/" + calendar.get(Calendar.DAY_OF_MONTH) + "/";
    System.out.println(" ccccc==sStyleUploadDir"+sStyleUploadDir);
    //sStyleUploadDir = bean.getSStyleUploadDir();
    //<add core end>
...
<Script Language=Javascript src="<%=request.getContextPath()%>/include/editor.js"></Script>
<Script Language=Javascript src="<%=request.getContextPath()%>/include/table.js"></Script>
<Script Language=Javascript src="<%=request.getContextPath()%>/include/menu.js"></Script>
            ...
            <table border=0 cellpadding=0 cellspacing=0 height=20>
              <tr>
                ...
                <td class=StatusBarBtnOff id=eWebEditor_CODE onclick="setMode('CODE')">
                  <img border=0 src="buttonimage/<%=sStyleDir%>/modecode.gif" width=50 height=15 align=absmiddle>
                </td>
                <td width=5>                </td>
                <td class=StatusBarBtnOff id=eWebEditor_EDIT onclick="setMode('EDIT')">
                  <img border=0 src="buttonimage/<%=sStyleDir%>/modeedit.gif" width=50 height=15 align=absmiddle>
                </td>
                <td width=5>                </td>
                <td class=StatusBarBtnOff id=eWebEditor_VIEW onclick="setMode('VIEW')">
                  <img border=0 src="buttonimage/<%=sStyleDir%>/modepreview.gif" width=50 height=15 align=absmiddle>
                </td>
              </tr>
            </table>
          </td>
            ...
            <table border=0 cellpadding=0 cellspacing=0 height=20>
              <tr>
                <td style="cursor:pointer;" onclick="sizeChange(300)">
                  <img border=0 SRC="buttonimage/<%=sStyleDir%>/sizeplus.gif" width=20 height=20 alt="增高编辑区">
                </td>
                <td width=5>                </td>
                <td style="cursor:pointer;" onclick="sizeChange(-300)">
                  <img border=0 SRC="buttonimage/<%=sStyleDir%>/sizeminus.gif" width=20 height=20 alt="减小编辑区">
                </td>
                <td width=40>                </td>
              </tr>
            </table>
...
<script>
window.setTimeout("setMode('EDIT'); ",300);//地址被选中初始化
